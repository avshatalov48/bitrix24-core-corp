...
*/
$arParams['MULTIPLE'] = isset($arParams['MULTIPLE']) && $arParams['MULTIPLE'] == 'Y' ? 'Y' : 'N'; // allow multiple user selection

$arParams['SHOW_BUTTON'] = isset($arParams['SHOW_BUTTON']) && $arParams['SHOW_BUTTON'] == 'N' ? 'N' : 'Y'; // show button for control run. Show() method should be used otherwise.
$arParams['SHOW_INPUT'] = isset($arParams['SHOW_INPUT']) && $arParams['SHOW_INPUT'] == 'Y' ? 'Y' : 'N'; // whether to show input field.
...
$arParams['IS_EXTRANET'] = isset($arParams['IS_EXTRANET']) && $arParams['IS_EXTRANET'] == 'Y' ? 'Y' : 'N'; // whether to show input field.

if (empty($arParams['INPUT_NAME']) && empty($arParams['ONSELECT']))
	return false;

$arParams['GET_FULL_INFO'] = isset($arParams['GET_FULL_INFO']) && $arParams['GET_FULL_INFO'] == 'Y' ? 'Y' : 'N'; // whether onselect handler should take full info. legal only with ONSELECT.
...
if ($arParams['MULTIPLE'] == 'N')
	$arParams['INPUT_VALUE'] = intval($arParams['INPUT_VALUE'] ?? 0);
elseif (isset($arParams['INPUT_VALUE']) && is_array($arParams['INPUT_VALUE']))
