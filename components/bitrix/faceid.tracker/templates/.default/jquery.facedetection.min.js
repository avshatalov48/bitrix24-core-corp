...
}]};try{var y=function(){"use strict";for(var n=document.getElementsByTagName("script"),p=0;p<n.length;p++)if(n[p].src.match(/(jquery\.facedetection(\.min)?\.js)|\/ccv\.js/))return n[p].src},x=y()}catch(e){}if(void 0===i){var i=function(n,p){"use strict";return i.core[p.toString()]=p().core,function(){var z,y,x,e;if(arguments.length>1)for(y=arguments[arguments.length-2],x=arguments[arguments.length-1],e=new Array(arguments.length-2),z=0;z<arguments.length-2;z++)e[z]=arguments[z];else y=arguments[0].async,x=arguments[0].worker,e=arguments[0],delete e.async,delete e.worker,e=[e];var i={shared:{}},s=p.apply(i,e);return y?function(y){var t=0,a=new Array(x),r=s.pre.apply(i,[x]);for(z in i.shared)"function"==typeof i.shared[z]?delete i.shared[z]:void 0!==i.shared[z].tagName&&delete i.shared[z];for(z=0;x>z;z++){var h=new Worker(n);h.onmessage=function(n){return function(p){a[n]="string"==typeof p.data?JSON.parse(p.data):p.data,t++,t==x&&y(s.post.apply(i,[a]))}}(z);var o={input:r[z],name:p.toString(),shared:i.shared,id:z,worker:e.worker_num};try{h.postMessage(o)}catch(d){h.postMessage(JSON.stringify(o))}}}:s.post.apply(i,[[s.core.apply(i,[s.pre.apply(i,[1])[0],0,1])]])}};i.core={}}var s={pre:function(n){if("img"==n.tagName.toLowerCase()){var p=document.createElement("canvas");document.body.appendChild(n),p.width=n.offsetWidth,p.style.width=n.offsetWidth.toString()+"px",p.height=n.offsetHeight,p.style.height=n.offsetHeight.toString()+"px",document.body.removeChild(n);var z=p.getContext("2d");return z.drawImage(n,0,0),p}return n},grayscale:function(n){for(var p,z,y=n.getContext("2d"),x=y.getImageData(0,0,n.width,n.height),e=x.data,i=n.width*n.height*4;i>0;)e[i-=4]=e[p=i+1]=e[z=i+2]=.3*e[i]+.59*e[p]+.11*e[z];return y.putImageData(x,0,0),n},array_group:function(n,p){var z,y,x=new Array(n.length);for(z=0;z<n.length;z++)x[z]={parent:-1,element:n[z],rank:0};for(z=0;z<n.length;z++)if(x[z].element){for(var e=z;-1!=x[e].parent;)e=x[e].parent;for(y=0;y<n.length;y++)if(z!=y&&x[y].element&&p(x[z].element,x[y].element)){for(var i=y;-1!=x[i].parent;)i=x[i].parent;if(i!=e){x[e].rank>x[i].rank?x[i].parent=e:(x[e].parent=i,x[e].rank==x[i].rank&&x[i].rank++,e=i);for(var s,t=y;-1!=x[t].parent;)s=t,t=x[t].parent,x[s].parent=e;for(t=z;-1!=x[t].parent;)s=t,t=x[t].parent,x[s].parent=e}}}var a=new Array(n.length),r=0;for(z=0;z<n.length;z++){y=-1;var h=z;if(x[h].element){for(;-1!=x[h].parent;)h=x[h].parent;x[h].rank>=0&&(x[h].rank=~r++),y=~x[h].rank}a[z]=y}return{index:a,cat:r}},detect_objects:i(x,function(){function n(){var n=this.shared.canvas,p=this.shared.interval,z=this.shared.scale,y=this.shared.next,x=this.shared.scale_upto,e=new Array(4*(x+2*y)),i=new Array(4*(x+2*y));e[0]=n,i[0]={width:e[0].width,height:e[0].height,data:e[0].getContext("2d").getImageData(0,0,e[0].width,e[0].height).data};var s;for(s=1;p>=s;s++)e[4*s]=document.createElement("canvas"),e[4*s].width=Math.floor(e[0].width/Math.pow(z,s)),e[4*s].height=Math.floor(e[0].height/Math.pow(z,s)),e[4*s].getContext("2d").drawImage(e[0],0,0,e[0].width,e[0].height,0,0,e[4*s].width,e[4*s].height),i[4*s]={width:e[4*s].width,height:e[4*s].height,data:e[4*s].getContext("2d").getImageData(0,0,e[4*s].width,e[4*s].height).data};for(s=y;x+2*y>s;s++)e[4*s]=document.createElement("canvas"),e[4*s].width=Math.floor(e[4*s-4*y].width/2),e[4*s].height=Math.floor(e[4*s-4*y].height/2),e[4*s].getContext("2d").drawImage(e[4*s-4*y],0,0,e[4*s-4*y].width,e[4*s-4*y].height,0,0,e[4*s].width,e[4*s].height),i[4*s]={width:e[4*s].width,height:e[4*s].height,data:e[4*s].getContext("2d").getImageData(0,0,e[4*s].width,e[4*s].height).data};for(s=2*y;x+2*y>s;s++)e[4*s+1]=document.createElement("canvas"),e[4*s+1].width=Math.floor(e[4*s-4*y].width/2),e[4*s+1].height=Math.floor(e[4*s-4*y].height/2),e[4*s+1].getContext("2d").drawImage(e[4*s-4*y],1,0,e[4*s-4*y].width-1,e[4*s-4*y].height,0,0,e[4*s+1].width-2,e[4*s+1].height),i[4*s+1]={width:e[4*s+1].width,height:e[4*s+1].height,data:e[4*s+1].getContext("2d").getImageData(0,0,e[4*s+1].width,e[4*s+1].height).data},e[4*s+2]=document.createElement("canvas"),e[4*s+2].width=Math.floor(e[4*s-4*y].width/2),e[4*s+2].height=Math.floor(e[4*s-4*y].height/2),e[4*s+2].getContext("2d").drawImage(e[4*s-4*y],0,1,e[4*s-4*y].width,e[4*s-4*y].height-1,0,0,e[4*s+2].width,e[4*s+2].height-2),i[4*s+2]={width:e[4*s+2].width,height:e[4*s+2].height,data:e[4*s+2].getContext("2d").getImageData(0,0,e[4*s+2].width,e[4*s+2].height).data},e[4*s+3]=document.createElement("canvas"),e[4*s+3].width=Math.floor(e[4*s-4*y].width/2),e[4*s+3].height=Math.floor(e[4*s-4*y].height/2),e[4*s+3].getContext("2d").drawImage(e[4*s-4*y],1,1,e[4*s-4*y].width-1,e[4*s-4*y].height-1,0,0,e[4*s+3].width-2,e[4*s+3].height-2),i[4*s+3]={width:e[4*s+3].width,height:e[4*s+3].height,data:e[4*s+3].getContext("2d").getImageData(0,0,e[4*s+3].width,e[4*s+3].height).data};return[i]}function z(n){var p,z,y,x,e,i,s=this.shared.cascade,t=(this.shared.interval,this.shared.scale),a=this.shared.next,r=this.shared.scale_upto,h=1,o=1,d=[0,1,0,1],c=[0,0,1,1],g=[];for(p=0;r>p;p++){var f=n[4*p+8*a].width-Math.floor(s.width/4),l=n[4*p+8*a].height-Math.floor(s.height/4),u=[4*n[4*p].width,4*n[4*p+4*a].width,4*n[4*p+8*a].width],w=[16*n[4*p].width-16*f,8*n[4*p+4*a].width-8*f,4*n[4*p+8*a].width-4*f];for(z=0;z<s.stage_classifier.length;z++){var v=s.stage_classifier[z].orig_feature,m=s.stage_classifier[z].feature=new Array(s.stage_classifier[z].count);for(y=0;y<s.stage_classifier[z].count;y++)for(m[y]={size:v[y].size,px:new Array(v[y].size),pz:new Array(v[y].size),nx:new Array(v[y].size),nz:new Array(v[y].size)},i=0;i<v[y].size;i++)m[y].px[i]=4*v[y].px[i]+v[y].py[i]*u[v[y].pz[i]],m[y].pz[i]=v[y].pz[i],m[y].nx[i]=4*v[y].nx[i]+v[y].ny[i]*u[v[y].nz[i]],m[y].nz[i]=v[y].nz[i]}for(i=0;4>i;i++){var _=[n[4*p].data,n[4*p+4*a].data,n[4*p+8*a+i].data],b=[8*d[i]+c[i]*n[4*p].width*8,4*d[i]+c[i]*n[4*p+4*a].width*4,0];for(e=0;l>e;e++){for(x=0;f>x;x++){var M=0,k=!0;for(z=0;z<s.stage_classifier.length;z++){M=0;var C=s.stage_classifier[z].alpha,m=s.stage_classifier[z].feature;for(y=0;y<s.stage_classifier[z].count;y++){var I,A,D=m[y],N=_[D.pz[0]][b[D.pz[0]]+D.px[0]],j=_[D.nz[0]][b[D.nz[0]]+D.nx[0]];if(j>=N)M+=C[2*y];else{var E,S=!0;for(E=0;E<D.size;E++){if(D.pz[E]>=0&&(I=_[D.pz[E]][b[D.pz[E]]+D.px[E]],N>I)){if(j>=I){S=!1;break}N=I}if(D.nz[E]>=0&&(A=_[D.nz[E]][b[D.nz[E]]+D.nx[E]],A>j)){if(A>=N){S=!1;break}j=A}}M+=S?C[2*y+1]:C[2*y]}}if(M<s.stage_classifier[z].threshold){k=!1;break}}k&&g.push({x:(4*x+2*d[i])*h,y:(4*e+2*c[i])*o,width:s.width*h,height:s.height*o,neighbor:1,confidence:M}),b[0]+=16,b[1]+=8,b[2]+=4}b[0]+=w[0],b[1]+=w[1],b[2]+=w[2]}}h*=t,o*=t}return g}function y(n){{var p,z,y=this.shared.min_neighbors,x=this.shared.cascade;this.shared.interval,this.shared.scale,this.shared.next,this.shared.scale_upto}for(p=0;p<x.stage_classifier.length;p++)x.stage_classifier[p].feature=x.stage_classifier[p].orig_feature;if(n=n[0],y>0){var e=s.array_group(n,function(n,p){var z=Math.floor(.25*n.width+.5);return p.x<=n.x+z&&p.x>=n.x-z&&p.y<=n.y+z&&p.y>=n.y-z&&p.width<=Math.floor(1.5*n.width+.5)&&Math.floor(1.5*p.width+.5)>=n.width}),i=e.cat,t=e.index,a=new Array(i+1);for(p=0;p<a.length;p++)a[p]={neighbors:0,x:0,y:0,width:0,height:0,confidence:0};for(p=0;p<n.length;p++){var r=n[p],h=t[p];0==a[h].neighbors&&(a[h].confidence=r.confidence),++a[h].neighbors,a[h].x+=r.x,a[h].y+=r.y,a[h].width+=r.width,a[h].height+=r.height,a[h].confidence=Math.max(a[h].confidence,r.confidence)}var o=[];for(p=0;i>p;p++){var d=a[p].neighbors;d>=y&&o.push({x:(2*a[p].x+d)/(2*d),y:(2*a[p].y+d)/(2*d),width:(2*a[p].width+d)/(2*d),height:(2*a[p].height+d)/(2*d),neighbors:a[p].neighbors,confidence:a[p].confidence})}var c=[];for(p=0;p<o.length;p++){var r=o[p],g=!0;for(z=0;z<o.length;z++){var f=o[z],l=Math.floor(.25*f.width+.5);if(p!=z&&r.x>=f.x-l&&r.y>=f.y-l&&r.x+r.width<=f.x+f.width+l&&r.y+r.height<=f.y+f.height+l&&(f.neighbors>Math.max(3,r.neighbors)||r.neighbors<3)){g=!1;break}}g&&c.push(r)}return c}return n}if(void 0!==this.shared){var x=p(arguments,["canvas","cascade","interval","min_neighbors"]);this.shared.canvas=x.canvas,this.shared.interval=x.interval,this.shared.min_neighbors=x.min_neighbors,this.shared.cascade=x.cascade,this.shared.scale=Math.pow(2,1/(x.interval+1)),this.shared.next=x.interval+1,this.shared.scale_upto=Math.floor(Math.log(Math.min(x.canvas.width/x.cascade.width,x.canvas.height/x.cascade.height))/Math.log(this.shared.scale));var e;for(e=0;e<this.shared.cascade.stage_classifier.length;e++)this.shared.cascade.stage_classifier[e].orig_feature=this.shared.cascade.stage_classifier[e].feature}return{pre:n,core:z,post:y}})};onmessage=function(n){var p="string"==typeof n.data?JSON.parse(n.data):n.data,z={shared:p.shared},y=i.core[p.name].apply(z,[p.input,p.id,p.worker]);try{postMessage(y)}catch(x){postMessage(JSON.stringify(y))}},n.fn.faceDetection=function(p){"use strict";var y,x={interval:4,minNeighbors:1,grayscale:!0,confidence:null,async:!1,complete:function(){},error:function(){}};return n.isFunction(p)?x.complete=p:n.extend(x,p),this.each(function(){function p(){var n,p;if(y=(new Date).getTime(),i.is("img"))n=new Image,n.src=i.attr("src"),n.crossOrigin=i.attr("crossorigin"),p=s.pre(n);else if(i.is("video")||i.is("canvas")){var t,a;n=i[0],t=document.createElement("canvas"),t.setAttribute("width",n.videoWidth||n.width),t.setAttribute("height",n.videoHeight||n.height),a=t.getContext("2d"),a.drawImage(n,0,0),p=s.pre(t)}x.grayscale&&(p=s.grayscale(p));try{x.async&&window.Worker?s.detect_objects({canvas:p,cascade:z,interval:x.interval,min_neighbors:x.minNeighbors,worker:1,async:!0})(e):e(s.detect_objects({canvas:p,cascade:z,interval:x.interval,min_neighbors:x.minNeighbors}))}catch(r){x.error.apply(i,[2,r.message]),x.complete.apply(i,[!1])}}function e(n){for(var p=n.length,z=[],e=0;p>e;++e)null!==x.confidence&&n[e].confidence<=x.confidence||(n[e].positionX=a.left+n[e].x,n[e].positionY=a.top+n[e].y,n[e].offsetX=t.left+n[e].x,n[e].offsetY=t.top+n[e].y,n[e].scaleX=r,n[e].scaleY=h,z.push(n[e]));z.time=(new Date).getTime()-y,x.complete.apply(i,[z])}var i=n(this),t=i.offset(),a=i.position(),r=i.width()/(this.naturalWidth||this.videoWidth)||1,h=i.height()/(this.naturalHeight||this.videoHeight)||1;return i.is("img, video, canvas")?p():(x.error.apply(i,[1,"Face detection is possible on images, videos and canvas only."]),void x.complete.apply(i,[[]]))})}}("function"==typeof jQuery?jQuery:"function"==typeof Zepto?Zepto:{fn:{}});
//# sourceMappingURL=jquery.facedetection.map.js