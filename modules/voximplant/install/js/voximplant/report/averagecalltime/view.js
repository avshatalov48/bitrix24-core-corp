;(function()
{
	...
	BX.Voximplant.Report.Dashboard.Content.AverageCallTime = function ()
	{
		BX.Report.VisualConstructor.Widget.Content.AmCharts4.apply(this, arguments);
	};

	BX.Voximplant.Report.Dashboard.Content.AverageCallTime.prototype = {

		__proto__: BX.Report.VisualConstructor.Widget.Content.AmCharts4.prototype,
		constructor: BX.Voximplant.Report.Dashboard.Content.AverageCallTime,
		...
		onAfterChartCreate: function ()
		{
			...
			if (this.chart.data.length > 5)
			{
				...
				scrollbar.background.fill = "#00DEBA";
				scrollbar.background.fillOpacity = 0.12;
				scrollbar.thumb.background.states.getKey('hover').properties.fill = am4core.color("#00DEBA");
			...
			{
				return (text === "0.1" ? "0" : text + ' ' + BX.message('TELEPHONY_REPORT_AVG_CALL_TIME_MIN'))
			})
		},
		...
		renderBalloon: function(text, target)
		{
			...
			return '<div class="telephony-report-avg-call-time-modal" style="border-color:' + target.fill.hex +'">' +
						'<div class="telephony-report-avg-call-time-modal-title">' +
							BX.util.htmlspecialchars(target.dataItem.dataContext.groupingField) +
						'</div>' +
						'<div class="telephony-report-avg-call-time-modal-subtitle-secondary">' +
							BX.message('TELEPHONY_REPORT_AVG_CALL_TIME') + '<br>' +
						'</div>' +
						'<div class="telephony-report-avg-call-time-modal-main">' +
							'<div class="telephony-report-avg-call-time-modal-content">' +
								'<div class="telephony-report-avg-call-time-modal-value">' +
									count['value_1'] +
								'</div>' +
								this.renderPercentBlock(compare['value_1'], compare['value_1_formatted']) +
							'</div>' +
						'</div>'+
					'</div>';
		},
