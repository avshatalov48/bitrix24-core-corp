;(function()
{
	...
	BX.Voximplant.Report.Dashboard.Content.CallDuration = function ()
	{
		BX.Report.VisualConstructor.Widget.Content.AmCharts4.apply(this, arguments);
	};

	BX.Voximplant.Report.Dashboard.Content.CallDuration.prototype = {

		__proto__: BX.Report.VisualConstructor.Widget.Content.AmCharts4.prototype,
		constructor: BX.Voximplant.Report.Dashboard.Content.CallDuration,
		...
		onAfterChartCreate: function()
		{
			...
			if (this.chart.yAxes.getIndex(0).logarithmic)
			{
				this.chart.yAxes.getIndex(0).renderer.labels.template.adapter.add("text", function(text)
				{
					return (text === "0.1" ? "0" : text + ' ' + BX.message('TELEPHONY_REPORT_CALL_DURATION_HOUR'));
				})
			}
		...
		renderBalloon: function(text, target)
		{
			...
			return '<div class="telephony-report-call-duration-modal" style="border-color: #2fc6f5">' +
						'<div class="telephony-report-call-duration-modal-title">' +
							BX.util.htmlspecialchars(target.dataItem.dataContext.groupingField) +
						'</div>' +
						'<div class="telephony-report-call-duration-modal-subtitle-secondary">' +
						BX.message('TELEPHONY_REPORT_CALL_DURATION_INCOMING') + '<br>' +
						'</div>' +
						'<div class="telephony-report-call-duration-modal-main">' +
							'<div class="telephony-report-call-duration-modal-content">' +
								'<div class="telephony-report-call-duration-modal-value">' +
									count['value_1'] +
								'</div>' +
						...
						'<div class="telephony-report-call-duration-modal-subtitle-secondary">' +
						BX.message('TELEPHONY_REPORT_CALL_DURATION_OUTGOING') + '<br>' +
						'</div>' +
						'<div class="telephony-report-call-duration-modal-main">' +
							'<div class="telephony-report-call-duration-modal-content">' +
								'<div class="telephony-report-call-duration-modal-value">' +
		...
		renderPercentBlock: function(value)
		{
			...
			var classList = "telephony-report-call-duration-modal-percent-value";
