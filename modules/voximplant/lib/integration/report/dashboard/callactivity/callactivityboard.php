...
namespace Bitrix\Voximplant\Integration\Report\Dashboard\CallActivity;
...
class CallActivityBoard
{
	public const VERSION = 'v4';
	public const BOARD_KEY = 'telephony_call_activity';
	...
	public static function get(): Dashboard
	{
		...
		$chart = static::buildCallActivityGraph();
		...
		$grid = static::buildCallActivityGrid();
	 ...
	 * @return Widget
	 */
	protected static function buildCallActivityGrid(): Widget
	{
		$widget = new Widget();
		$widget->setGId(Util::generateUserUniqueId());
		$widget->setWidgetClass(BaseWidget::getClassName());
		$widget->setViewKey(View\CallActivity\CallActivityGrid::VIEW_KEY);
		$widget->setCategoryKey('telephony');
		$widget->setBoardId(static::BOARD_KEY);
		$widget->getWidgetHandler(true)
			   ->updateFormElementValue('label', Loc::getMessage('TELEPHONY_REPORT_CALL_ACTIVITY_DAY'));
		$widget->addConfigurations($widget->getWidgetHandler(true)->getConfigurations());

		$report = new Report();
		$report->setGId(Util::generateUserUniqueId());
		$report->setReportClassName(Handler\CallActivity\CallActivityGrid::class);
		$report->setWidget($widget);
		$report->addConfigurations($report->getReportHandler(true)->getConfigurations());
		$widget->addReports($report);
