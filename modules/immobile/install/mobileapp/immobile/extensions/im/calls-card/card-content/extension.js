/**
 * @module im/calls-card/card-content
 */
jn.define('im/calls-card/card-content', (require, exports, module) => {
	const { PhoneField } = require('layout/ui/fields/phone');
	const { Alert } = require('alert');

	const { Overlay } = require('im/calls-card/card-content/elements');
	const { CollapseButton } = require('im/calls-card/card-content/elements');
	const { Avatar } = require('im/calls-card/card-content/elements');
	const { Status } = require('im/calls-card/card-content/elements');
	const { CrmButton } = require('im/calls-card/card-content/elements');
	const { Button }  = require('im/calls-card/card-content/elements');
	const { Timer } = require('im/calls-card/card-content/elements');
	const { CallControls } = require('im/calls-card/card-content/elements');
	...
	class CardContent extends PureComponent
	{
		...
		renderButtons()
		{
			return View(
				...
				new Button({
					...
					text: BX.message('IMMOBILE_CALLS_CARD_MICROPHONE_BUTTON'),
					isSwitchable: true,
				...
				new Button({
					icon: icons.keyboard,
					text: BX.message('IMMOBILE_CALLS_CARD_KEYBOARD_BUTTON'),
				...
				new Button({
					icon: icons.pause,
					text: BX.message('IMMOBILE_CALLS_CARD_PAUSE_BUTTON'),
					isSwitchable: true,
				...
				new Button({
					icon: icons.dynamic,
					text: BX.message('IMMOBILE_CALLS_CARD_DYNAMIC_BUTTON'),
					isSwitchable: true,
		...
		showError()
		{
			Alert.alert(BX.message('IMMOBILE_CALLS_CARD_ERROR_TITLE'), this.state.errorText);
		}
	}
