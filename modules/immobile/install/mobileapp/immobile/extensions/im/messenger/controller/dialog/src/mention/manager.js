/**
 * @module im/messenger/controller/dialog/mention/manager
 */
jn.define('im/messenger/controller/dialog/mention/manager', (require, exports, module) => {
	const { MentionProvider } = require('im/messenger/controller/dialog/mention/provider');
	const { ChatAvatar } = require('im/messenger/lib/element');
	const { ChatTitle } = require('im/messenger/lib/element');
	const { EventType } = require('im/messenger/const');
	const { DialogHelper } = require('im/messenger/lib/helper');
	const { BBCode } = require('im/messenger/const');
	...
	class MentionManager
	{
		 ...
		 */
		onProcessedMentionChangeText(text, inputCharacters, cursorPosition)
		{
			...
			{
				const userIds = this.provider.loadRecentUsers();
				this.drawItems(userIds);
		 ...
		 * @param cursorPosition
		 */
		onInactiveMentionChangeText(text, inputCharacters, cursorPosition)
		{
		...
		drawItems(itemIds)
		{
			...
			itemIds.forEach((itemId) => {
				...
				const recentItem = core.getStore().getters['recentModel/getById'](item.id)
					?? core.getStore().getters['recentModel/searchModel/getById'](item.id)
				;
