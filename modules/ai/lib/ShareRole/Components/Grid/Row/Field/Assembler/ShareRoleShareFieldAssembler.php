...
class ShareRoleShareFieldAssembler extends FieldAssembler
{
	protected function prepareColumn($value): string
	{
		$shares = array_values($value['shares']);
		$shareRoleCode = $value['roleCode'];
		$totalCount = $value['totalCount'] ?? 0;

		$firstValues = array_slice($shares, 0, 5);

		$result = '<div class="ai__role-library-grid_shares-cell">';
		...
		if ($totalCount > count($shares))
		{
			...
			$onClick = "BX.AI.ShareRole.Library.Controller.handleClickOnSharesCell('{$shareRoleCode}', event)";
			$result .= '
				<div class="ai__role-library-grid_shares-etc-items-count" title="Показать все" onclick="'
				. $onClick
				. '">+'
				. $otherSharesCount
				. '</div>';
	...
	protected function getAvatarWithPhoto(ShareDto $shareDto): string
	{
		...
		return "
			<div data-hint='{$name}' data-hint-no-icon class='ai__role-library-grid_shares-item'>
	...
	protected function getAvatarWithInitials(ShareDto $shareDto): string
	{
		...
		return
			"<div data-hint='{$fullName}' data-hint-no-icon class='ai__role-library-grid_shares-item'>
				<span class='ai__role-library-grid_share-initials'>
					{$initials}
				</span>
			</div>"
			;
