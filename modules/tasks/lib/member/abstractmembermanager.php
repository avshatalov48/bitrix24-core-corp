...
abstract class AbstractMemberManager
{
	private static array $cache = [];

	public function __construct(private RepositoryInterface $repository, private ConfigInterface $config)
	{
	}
	...
	public function get(): array
	{
		...
		return match ($this->config::class)
		{
			...
			WorkConfig::class => $this->getWorkInfo(),
	...
	private function getKey(ConfigInterface $config): string
	{
		return implode('|', [
	 ...
	 * @throws UserInfoException
	 */
	private function getWorkInfo(): array
	{
		$members = $this->repository->getMembers();
		if ($members->isEmpty())
		{
		...
		try
		{
			$query = UserTable::query();
			$query
				->setSelect(['ID', 'NAME', 'LAST_NAME', 'WORK_POSITION'])
				->whereIn('ID', $memberIds)
			;
