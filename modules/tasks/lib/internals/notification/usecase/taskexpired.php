...
class TaskExpired
{
	...
	private function expiredForResponsible(User $sender, ProviderInterface $provider): void
	{
	...
	private function expiredForAccomplices(User $sender, ProviderInterface $provider): void
	{
	...
	private function expiredForCreator(User $sender, ProviderInterface $provider): void
	{
		...
		$recepient = $this->userRepository->getUserById($this->task->getCreatedBy());
		if (!$recepient)
		{
			return;
		}
		...
		$provider->addMessage(new Message(
			...
			$this->getMetadata([
				'task' => $this->task,
				'member_code' => RoleDictionary::ROLE_DIRECTOR
			])
		));
	}
