	...
	public static function prepare(array $row, array $parameters)
	{
		...
		if ($row['ACTION']['EDIT'])
		{
			...
			<a class="task-grade-and-report <?=$markClass?> <?=$addInReportClass?>" href="javascript: void(0)"
			   ...
			   title="<?=Loc::getMessage('TASKS_GRID_ROW_CONTENT_MARK')?>: <?=Loc::getMessage('TASKS_GRID_ROW_CONTENT_MARK_'.($mark === "N" || $mark === "P" ? $mark : "NONE"))?>">
		...
		else
		{
			...
			<span class="<?=$markClass?> <?=$addInReportClass?>" href="javascript: void(0)"
				  title="<?=Loc::getMessage('TASKS_GRID_ROW_CONTENT_MARK')?>: <?=Loc::getMessage('TASKS_GRID_ROW_CONTENT_MARK_'.($mark ?: 'NONE'))?>">
