<?php
namespace Bitrix\Tasks\Grid\Row\Content\UserField;
 ...
 * Class Crm
 *
 * @package Bitrix\Tasks\Grid\Row\Content\UserField
 */
class Crm extends UserField
{
	public static function prepare(array $row, array $parameters): string
	{
		...
		{
			return Loc::getMessage('TASKS_GRID_ROW_CONTENT_USER_FIELD_CRM_NOT_PRESENT');
		}
			...
			foreach ($collection as $type => $items)
			{
				...
				if ($type !== $previousType)
				{
					$html[] = '<span class="tasks-list-crm-div-type">'
						.Loc::getMessage('TASKS_GRID_ROW_CONTENT_USER_FIELD_CRM_CRM_TYPE_'.$type)
						.'</span>';
				}
				$html[] = implode(', ', $items);
				$html[] = '</div>';
