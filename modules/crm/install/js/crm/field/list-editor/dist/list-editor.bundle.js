...
(function (exports,landing_ui_panel_fieldspanel,ui_draganddrop_draggable,ui_sidepanel_layout,ui_buttons,main_loader,main_core_events,ui_forms,_ui_layoutForm,main_core) {
	'use strict';
	...
	class ListEditor extends main_core_events.EventEmitter {
	  constructor(options = {}) {
	    ...
	    this.setOptions({ ...babelHelpers.classPrivateFieldLooseBase(ListEditor, _defaultOptions)[_defaultOptions],
	      ...options
	    });
	  ...
	  setData(data) {
	    babelHelpers.classPrivateFieldLooseBase(this, _cache$2)[_cache$2].set('data', { ...data
	    });
	  }
	    ...
	    BX.SidePanel.Instance.open('crm:field-list-editor', {
	      ...
	      contentCallback: () => {
	        return babelHelpers.classPrivateFieldLooseBase(this, _loadPromise)[_loadPromise].then(() => ui_sidepanel_layout.Layout.createContent({
	          ...
	          content: () => {
	            const title = main_core.Loc.getMessage('CRM_FIELD_LIST_EDITOR_ERROR_LOAD');
	...
	function _adjustSliderDragAndDropOffsets2() {
	  ...
	  if (main_core.Type.isDomNode(sliderLayout)) {
	    ...
	    this.draggable.setOptions({ ...this.draggable.getOptions(),
	      offset: {
	        x: offsetLeft
	      }
	    });
...
}((this.BX.Crm.Field = this.BX.Crm.Field || {}),BX.Landing.UI.Panel,BX.UI.DragAndDrop,BX.UI.SidePanel,BX.UI,BX,BX.Event,BX,BX,BX));
