	...
	<tr><?
		...
		<td><?
			...
			<? if (CSalePaySystemAction::GetParamValue("SELLER_PHONE", false)) { ?>
				<?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_PHONE', null, $lng).": %s", htmlspecialcharsbx(CSalePaySystemAction::GetParamValue("SELLER_PHONE", false))); ?><br>
			<? } ?>
			<? if (CSalePaySystemAction::GetParamValue("SELLER_EMAIL", false)) { ?>
				<?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_EMAIL', null, $lng).": %s", htmlspecialcharsbx(CSalePaySystemAction::GetParamValue("SELLER_EMAIL", false))); ?>
			<? } ?>
		</td>
	</tr>
</table>
...
<div style="margin: 0pt; padding: 0pt;">
	<? if (CSalePaySystemAction::GetParamValue("BUYER_NAME", false)) { ?>
		...
		if ($buyerPayerName)
		{
			?><?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_CONTACT_PERSON', null, $lng).": %s", htmlspecialcharsbx($buyerPayerName, ENT_COMPAT, false)); ?><br><?
		}
		unset($buyerPayerName);
		$buyerPhone = CSalePaySystemAction::GetParamValue("BUYER_PHONE", false);
		if ($buyerPhone)
		{
			?><?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_PHONE', null, $lng).": %s", htmlspecialcharsbx($buyerPhone)); ?><br><?
		}
		unset($buyerPhone);
		$buyerFax = CSalePaySystemAction::GetParamValue("BUYER_FAX", false);
		if ($buyerFax)
		{
			?><?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_FAX', null, $lng).": %s", htmlspecialcharsbx($buyerFax)); ?><br><?
		}
		unset($buyerFax);
		$buyerEmail = CSalePaySystemAction::GetParamValue("BUYER_EMAIL", false);
		if ($buyerEmail)
		{
			?><?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_EMAIL', null, $lng).": %s", htmlspecialcharsbx($buyerEmail)); ?><br><?
		}
		unset($buyerEmail);
		?>
	<? } ?>
	...
	<tr>
		...
		<td align="right">
			<?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_ISSUE_DATE', null, $lng).': %s',
				htmlspecialcharsbx(CSalePaySystemAction::GetParamValue("DATE_INSERT", false))); ?>
		</td>
	</tr>
</table>
<br>
...
if (!empty($arBasketItems))
{
	...
	foreach ($columnList as $column)
	{
		...
		{
			$arCols[$column] = array(
				'NAME' => htmlspecialcharsbx(CSalePaySystemAction::GetParamValue('QUOTE_FR_COLUMN_'.$column.'_TITLE'), ENT_COMPAT, false),
