...
<div style="margin: 0pt; padding: 0pt;">
	<? if (CSalePaySystemAction::GetParamValue("BUYER_NAME", false)) { ?>
		...
		if ($buyerPayerName)
		{
			?><?=sprintf(Loc::getMessage('SBLP_Q_BR_TEXT_CONTACT_PERSON', null, $lng).": %s", htmlspecialcharsbx($buyerPayerName)); ?><br><?
		}
...
if (!empty($arBasketItems))
{
	...
	foreach ($columnList as $column)
	{
		...
		{
			$arCols[$column] = array(
				'NAME' => htmlspecialcharsbx(CSalePaySystemAction::GetParamValue('QUOTE_BR_COLUMN_'.$column.'_TITLE')),
