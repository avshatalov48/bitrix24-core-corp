(function (exports,main_core,main_popup,ui_dialogs_messagebox) {
	...
	function _renderEntities2() {
	  ...
	  buttons.forEach(function (node) {
	    ...
	    var handler = function handler(event) {
	      ...
	      var contentNode = main_core.Tag.render(_templateObject || (_templateObject = babelHelpers.taggedTemplateLiteral(["<div class=\"crm-webform-list-entities\"></div>"])));
	      data.counters.forEach(function (counter) {
	        ...
	        var counterNode = !counter.LINK ? main_core.Tag.render(_templateObject2 || (_templateObject2 = babelHelpers.taggedTemplateLiteral(["\n\t\t\t\t\t\t\t<span \n\t\t\t\t\t\t\t\tclass=\"crm-webform-active-popup-item-date\" \n\t\t\t\t\t\t\t\ttitle=\"", "\"\n\t\t\t\t\t\t\t>", "</span>\n\t\t\t\t\t\t"])), caption, caption) : main_core.Tag.render(_templateObject3 || (_templateObject3 = babelHelpers.taggedTemplateLiteral(["\n\t\t\t\t\t\t\t<a\n\t\t\t\t\t\t\t\thref=\"", "\"\n\t\t\t\t\t\t\t\tonclick=\"", "\"\n\t\t\t\t\t\t\t\tclass=\"crm-webform-active-popup-item-date\"\n\t\t\t\t\t\t\t\ttitle=\"", "\"\n\t\t\t\t\t\t\t>", "</a>\n\t\t\t\t\t\t"])), main_core.Text.encode(counter.LINK), counterHandler, caption, caption);
	        contentNode.appendChild(main_core.Tag.render(_templateObject4 || (_templateObject4 = babelHelpers.taggedTemplateLiteral(["\n\t\t\t\t\t\t<div class=\"crm-webform-list-active-popup-row\">\n\t\t\t\t\t\t\t", "\n\t\t\t\t\t\t\t<span class=\"crm-webform-list-entity-counter\">", "</span>\n\t\t\t\t\t\t</div>\t\t\t\t\t\t\n\t\t\t\t\t"])), counterNode, value));
	      });
	      var popupWidth = 160;
	...
	function _renderActiveSwitchers2() {
	  ...
	  switchers.forEach(function (node) {
	    node.innerHTML = '';
	    var data = JSON.parse(node.getAttribute(switcherAttr));
	    var nodeText = main_core.Tag.render(_templateObject5 || (_templateObject5 = babelHelpers.taggedTemplateLiteral(["\n\t\t\t\t<div\n\t\t\t\t\tclass=\"", "\"\n\t\t\t\t>", "</div>\n\t\t\t"])), data.active ? '' : 'crm-webform-list-text-gray', main_core.Text.encode(data.dateActiveShort));
	    var switcher = new BX.UI.Switcher({
	      ...
	      handlers: {
	        toggled: function toggled() {
	          _this9.activate(data.id, switcher.isChecked(), false, nodeText);
...
}((this.window = this.window || {}),BX,BX.Main,BX.UI.Dialogs));
