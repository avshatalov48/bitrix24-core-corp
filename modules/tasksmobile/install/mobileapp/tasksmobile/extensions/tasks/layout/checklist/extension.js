...
jn.define('tasks/layout/checklist', (require, exports, module) => {
	...
	const pathToExtension = '/bitrix/mobileapp/tasksmobile/extensions/tasks/layout/checklist/';
	...
	class Input extends LayoutComponent
	{
		...
		addMember(member)
		{
			...
			this.props.onMemberSelected(
				member,
				currentTitle.substring(0, this.cursorPosition).trim()
		...
		getTitleWithUrls(title, membersMap)
		{
			...
			members.forEach(({id, nameFormatted, type}) => {
				...
				title = title.replace(
					regExp,
					'[COLOR=#2067b0][URL=' + id + ']' + nameFormatted + '[/URL][/COLOR]'
				);
			});
	...
	class List extends LayoutComponent
	{
		...
		onLayout({ height })
		{
			if (this.descendantsHeight !== height)
			{
				this.descendantsHeight = height;

				if (!this.isTogglingList)
				{
					if (this.state.isShown)
					{
						this.show();
		...
		calculateListHeight()
		{
			...
			return borderOffset + btnHeight + headerHeight + parseInt(this.descendantsHeight, 10);
		}
	}
			...
			return View(
				...
				View(
					...
					Image({
						...
						svg: {
							uri: `${currentDomain}${pathToExtension}images/arrow.svg?2`,
						},
						resizeMode: 'center',
						onClick: () => {
				...
				View(
					...
					Image({
						...
						svg: {
							uri: `${currentDomain}${pathToExtension}images/three-dots.svg?2`
						},
						resizeMode: 'center',
						onClick: () => {
	...
	class Item extends LayoutComponent
	{
			...
			return View(
				...
				View(
					...
					new ItemAttachments({
						...
						ref: (ref) => this.attachmentsRef = ref,
					}),
					new ItemSettings({
						checkList: this.props.checkList,
						parentWidget: this.props.parentWidget,
		...
		showList()
		{
			...
			actions.push({
				...
				data: {
					svg: {
						uri: `${currentDomain}${pathToExtension}images/new-list.svg?2`
					},
				},
				onClickCallback: () => {
					return new Promise((resolve) => {
			...
			for (let k = 0; k < depth; k++)
			{
				dots.push(
					Image({
						...
						svg: {
							uri: `${currentDomain}${pathToExtension}images/depth-dot.svg?2`
						},
						resizeMode: 'center',
					}),
				);
		...
		render()
		{
			return View(
				...
				Image({
					...
					svg: {
						uri: `${currentDomain}${pathToExtension}images/important-item.svg?2`
					},
					resizeMode: 'center'
				})
			)
	...
	class ItemAttachments extends LayoutComponent
	{
		...
		render()
		{
			return View(
				...
				Image({
					...
					svg: {
						uri: `${currentDomain}${pathToExtension}images/file-clip.svg?2`
					},
					resizeMode: 'center',
				}),
				Text({
		...
		onAttachFiles(eventData)
		{
			...
			const inputId = (nodeId ? nodeId : checkListItemId);
		...
		render()
		{
			return View(
				...
				Image({
					...
					svg: {
						uri: `${currentDomain}${pathToExtension}images/add.svg?2`
					},
				}),
				Text({
