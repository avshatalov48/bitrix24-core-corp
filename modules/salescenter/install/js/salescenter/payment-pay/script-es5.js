	...
	BX.SalesCenter.Component.PaymentPayBase.prototype =
		{
			...
			addReloadPageButton: function()
			{
				var resultDiv = BX.create('div', {
					...
					children: [
						...
						BX.create('div',{
							props: {className: 'order-basket-section-description py-3'},
						...
						BX.create('button', {
							...
							props: {
								className: 'order-payment-method-item-button btn btn-primary rounded-pill'
							},
							events: {
			...
			submit: function(e)
			{
				...
				if (!paysystemId || !this._isAllowedSubmitting)
				{
					if (selected)
					{
				...
				).then(BX.proxy(this.onAfterPay, this));
			},
			...
			onAfterPay: function(result)
			{
				...
				{
					var url = BX.type.isString(result.data.url) ? result.data.url : '',
						html = BX.type.isString(result.data.html) ? result.data.html : '';
					...
					else
					{
						if (html.length === 0)
						{
							...
							var fieldsNodes = [];
							if (fields && fields.SUM_WITH_CURRENCY)
							{
								fieldsNodes.push(BX.create('div',{
							...
							if (fields && fields.PAY_SYSTEM_NAME)
							{
								fieldsNodes.push(BX.create('div',{
									props: {className: 'checkout-basket-personal-order-info-item'},
	...
	BX.SalesCenter.Component.PaySystemItem.prototype =
		{
			...
			layout: function()
			{
				...
				var logoBlock, nameBlock;
				var logoImage = this._fields['LOGOTIP'];
				var payName = this._fields['NAME'];

				if (!logoImage)
				{
					logoImage = '/bitrix/js/salescenter/payment-pay/payment-method/images/default_logo.png';
				}

				logoBlock = BX.create('div', {
					props: {className: 'order-pay-method-item-logo-block'},
					children: [
						BX.create('div', {
							props: {className: 'order-pay-method-logo'},
							attrs: {
				...
				nameBlock = BX.create('div', {
					props: {className: 'order-pay-method-text-block'},
					children: [
						BX.create('div', {
							props: {className: 'order-pay-method-text'},
							text: payName
						})
					]
				});

				var btnClass = this._isViewMode ?
					'btn info-mode' :
					'order-payment-method-item-button btn btn-primary rounded-pill pay-mode';

				return BX.create('div', {
					props: {
						className: 'order-pay-method-item-container ' + additionalClass,
					},
					children: [
						logoBlock,
						nameBlock,
						BX.create('div', {
							props: {className: btnClass},
