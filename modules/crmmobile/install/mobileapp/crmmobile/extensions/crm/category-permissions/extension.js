...
jn.define('crm/category-permissions', (require, exports, module) => {
	...
	class CategoryPermissions extends LayoutComponent
	{
		...
		render()
		{
			return View(
				...
				View(
					...
					Text({
						style: styles.title,
						text: BX.message('M_CRM_CATEGORY_PERMISSION_TITLE2'),
					}),
					Text({
		...
		getCurrentAccessText()
		{
			...
			{
				return this.entityTypeId === 2 ?
					BX.message('M_CRM_CATEGORY_PERMISSION_DEAL_ALL_FOR_ALL')
					: BX.message('M_CRM_CATEGORY_PERMISSION_ITEM_ALL_FOR_ALL');
			}
			...
			{
				return this.entityTypeId === 2 ?
					BX.message('M_CRM_CATEGORY_PERMISSION_DEAL_OWN_FOR_ALL')
					: BX.message('M_CRM_CATEGORY_PERMISSION_ITEM_OWN_FOR_ALL');
			}
		...
		showMenu()
		{
			this.menu = new ContextMenu({
				...
				params: {
					title: BX.message('M_CRM_CATEGORY_PERMISSION_TITLE2'),
		...
		getMenuActions()
		{
			...
			return [
				...
				{
					id: 'noneForAll',
					title: BX.message('M_CRM_CATEGORY_PERMISSION_NONE_FOR_ALL'),
					sectionCode: 'managers',
				...
				},
				{
					...
					title: BX.message('M_CRM_CATEGORY_PERMISSION_MENU_COPY_FROM_TUNNELS2'),
					data: {
			...
			return new Promise((resolve, reject) => {
				...
				return (
					CategoryListView.open({
						entityTypeId,
						onSelectCategory: ({ id }) => this.setAccess(id).then(resolve),
						readOnly: true,
						enableSelect: true,
						showCounters: false,
						showTunnels: false,
	...
	const svgImages = {
		...
		arrow: `<svg width="10" height="18" viewBox="0 0 10 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path opacity="0.4" fill-rule="evenodd" clip-rule="evenodd" d="M0 2.34343L5.52705 7.87048L6.69963 8.99985L5.52705 10.1299L0 15.6569L1.59747 17.2544L9.85163 9.00025L1.59747 0.746094L0 2.34343Z" fill="#767C87"/></svg>`,
