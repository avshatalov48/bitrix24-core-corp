...
jn.define('crm/tunnel-list/item', (require, exports, module) => {
	...
	class TunnelListItem extends LayoutComponent
	{
		...
		prepareDelayText(delay)
		{
			...
			if (delay.type === DelayIntervalType.In)
			{
				...
				{
					if (delay.basis === basisFields[i].systemExpression)
					{
						str += ' ' + basisFields[i].name;
						break;
					}
				}
			}
			else if (delay.value)
			{
				...
				for (let i = 0; i < basisFields.length; ++i)
				{
					if (delay.basis === basisFields[i].systemExpression)
					{
						str += ' ' + fieldSuffix + ' ' + basisFields[i].name;
						break;
					}
				}
			}
		...
		prepareConditionText(conditionGroup)
		{
			...
			conditionGroup.items.forEach((item, index) => {
				if (item[0].field !== '')
				{
					...
					const valueLabel = (item[0].operator.indexOf('empty') < 0)
		...
		formatValuePrintable(property, value)
		{
			...
			switch (property['type'])
			{
				...
				case 'user':
					result = [];
					let i, name, pair, matches, pairs = Array.isArray(value) ? value : value.split(',');
	...
	const svgImages = {
		...
		menuIcon: `<svg width="17" height="4" viewBox="0 0 17 4" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.87097 1.93548C3.87097 3.00442 3.00442 3.87097 1.93548 3.87097C0.866546 3.87097 0 3.00442 0 1.93548C0 0.866546 0.866546 0 1.93548 0C3.00442 0 3.87097 0.866546 3.87097 1.93548Z" fill="#B9C0CA"/><path d="M8.12903 3.87097C9.19797 3.87097 10.0645 3.00442 10.0645 1.93548C10.0645 0.866546 9.19797 0 8.12903 0C7.06009 0 6.19355 0.866546 6.19355 1.93548C6.19355 3.00442 7.06009 3.87097 8.12903 3.87097Z" fill="#B9C0CA"/><path d="M14.3226 3.87097C15.3915 3.87097 16.2581 3.00442 16.2581 1.93548C16.2581 0.866546 15.3915 0 14.3226 0C13.2536 0 12.3871 0.866546 12.3871 1.93548C12.3871 3.00442 13.2536 3.87097 14.3226 3.87097Z" fill="#B9C0CA"/></svg>`,
