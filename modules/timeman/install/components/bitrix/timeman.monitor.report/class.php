...
class TimemanMonitorReportComponent extends CBitrixComponent implements Controllerable
{
	...
	protected function prepareReport(WorkingHoursReport $report): bool
	{
		...
		if (is_array($expandedRows))
		{
			...
			{
				$expandedUserIds[] = (int)explode('user-', $row)[1];
			}
		}
			...
			foreach ($timelineReportData as $userId => $dates)
			{
				foreach ($dates as $date => $desktops)
				{
					...
					$rows[] = [
						...
						'columns' => [
							'DATE' => (new Date($date, 'Y-m-d'))->toString(),
							'WORK_TIME' => $timelineLegendReportData[$userId][$date][$desktopCode]['formatted'],
							'CHART_DATA' => $this->prepareChartData(array_shift($desktops)),
							'USER_ID' => $userId,
							'DATE_LOG' => $date,
						],
						'actions' => [],
	...
	protected function prepareChartData(array $timelineReportData): array
	{
		if (!$timelineReportData)
		{
			return [];
		}
		...
		$intervals = array_map(
			static function ($interval) use ($dateFormat) {
				$interval['start'] = (new DateTime($interval['start']))->getTimestamp();
				$interval['finish'] = (new DateTime($interval['finish']))->getTimestamp();
		...
		$fixedInactiveIntervals = array_filter(
			$intervals,
			static function ($interval) {
				return (
					$interval['type'] === Group::INACTIVE
					&& $interval['time'] > self::ONE_HOUR
				);
			},
		);

		$totalDynamicTime = $totalTime - array_sum(array_column($fixedInactiveIntervals, 'time'));
		...
		foreach ($intervals as $index => $interval)
		{
			...
			$intervals[$index]['fixedSize'] = (
				$interval['type'] === Group::INACTIVE
				&& $interval['time'] > self::ONE_HOUR
			);

			if (!$intervals[$index]['fixedSize'])
			{
				$intervals[$index]['size'] = 100 / ($totalDynamicTime / $intervals[$index]['time']);
			}
			else
			{
				$intervals[$index]['size'] = null;
			}
		}
		...
		else if ($intervalsLength === 3)
		{
			$intervals[$intervalsLength - 1]['showStartMarker'] = true;
			$intervals[$intervalsLength - 2]['showStartMarker'] = true;
		}
