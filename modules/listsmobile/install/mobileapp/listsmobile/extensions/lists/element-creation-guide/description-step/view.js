/**
 * @module lists/element-creation-guide/description-step/view
 */
	...
	class DescriptionStepView extends PureComponent
	{
		...
		render()
		{
			return View(
				...
				this.props.isLoaded && this.renderDescription(),
			);
		}

		renderSkeleton()
		{
			return new DescriptionStepSkeleton({});
		}
		...
		renderBody()
		{
			...
			return View(
				...
				Text({
					testId: 'elementCreationGuide_DescriptionStep_title',
				...
				description && BBCodeText({
					testId: 'elementCreationGuide_DescriptionStep_description',
					value: description,
					style: {
						marginTop: 10,
						fontSize: 14,
		...
		renderStub()
		{
			return View(
				{ style: { height: 242 } },
				new Stub({
					image: { style: { width: 148, height: 149 } },
					styles: {
						container: {
		...
		renderExecuteTimeTitle()
		{
			...
			return View(
				...
				Text({
					...
					text: Loc.getMessage('LISTSMOBILE_EXT_ELEMENT_CREATION_GUIDE_DESCRIPTION_STEP_EXECUTE_TIME_TITLE'),
				}),
				View(
					{
						style: {
					...
					Text({
						...
						text: (
							formattedTime
							|| Loc.getMessage('LISTSMOBILE_EXT_ELEMENT_CREATION_GUIDE_DESCRIPTION_STEP_EXECUTE_TIME_UNDEFINED')
						),
						numberOfLines: 1,
						ellipsize: 'end',
					}),
			...
			return Text({
				...
				text: (
					this.props.formattedTime
						? Loc.getMessage('LISTSMOBILE_EXT_ELEMENT_CREATION_GUIDE_DESCRIPTION_STEP_EXECUTE_TIME_DESCRIPTION')
						: Loc.getMessage('LISTSMOBILE_EXT_ELEMENT_CREATION_GUIDE_DESCRIPTION_STEP_EXECUTE_TIME_UNDEFINED_DESCRIPTION')
				),
			});
		}
		...
		renderExecuteTimeLink()
		{
			return View(
				...
				Text({
					...
					text: Loc.getMessage('LISTSMOBILE_EXT_ELEMENT_CREATION_GUIDE_DESCRIPTION_STEP_TO_OPTIMIZE_LINK_TITLE'),
				}),
			);
		}
	}
