...
/*				if (ii != parseInt(ii))
					continue;
*/				_text = '<table border="0" cellpadding="0" cellspacing="0" class="wd-dropdown-item" ' + 
					'onMouseOver="this.className=\'wd-dropdown-item wd-dropdown-item-over\';" ' + 
					'onMouseOut="this.className=\'wd-dropdown-item\';"><tr>';
				...
				_table += '<tr class="wd-dropdown-menu' + (data[ii]['CLASS'] ? (" " + data[ii]['CLASS']) : "") + '" '
					...
					+ '<td class="wd-dropdown-menu' + (iCounter == 1 ? ' wd-dropdown-menu-first' : '') + '" ' 
	...
	this.PopupShow = function(pos, div, controlpos)
	{
		...
		jsFloatDiv.Show(this.oDiv, parseInt(pos["left"]), parseInt(pos["top"]), true);
		if (this.oControl != null && this.oControl.className)
		{
			this.oControl.className += ' wd-dropdown-pointer-active';
		}
	...
	{ 
		...
		if (this.oControl != null && this.oControl.className)
		{
			this.oControl.className = this.oControl.className.replace(" wd-dropdown-pointer-active", "");
		}
...
window.WDDropdownMenu = new WDDropdownMenu();