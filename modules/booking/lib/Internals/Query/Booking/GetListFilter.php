...
use Bitrix\Booking\Internals\Query\Filter;
...
class GetListFilter extends Filter implements FilterInterface
{
	...
	public function prepareQuery(Query $query): void
	{
		...
		)
		{
			$query->registerRuntimeField(
				...
				new ExpressionField(
					...
					"
						CASE WHEN (
							%s > " . (int)$this->currentTimestamp . "
						)
						THEN %s - " . (int)$this->currentTimestamp . "
						ELSE 0
						END
					",
					[
		...
		if (isset($this->filter['WITHIN_CURRENT_DAYTIME']))
		{
			$query->registerRuntimeField(
				...
				new ExpressionField(
					'CURRENT_HOUR',
					"EXTRACT(HOUR FROM FROM_UNIXTIME(" . $this->currentTimestamp . ") + INTERVAL %s SECOND)",
		...
		if (isset($this->filter['IS_SAME_DAY_OR_EARLY_MORNING_START']))
		{
			$query->registerRuntimeField(
				...
				new ExpressionField(
					...
					"
						CASE WHEN (
							EXTRACT(DAY FROM FROM_UNIXTIME(%s) + INTERVAL %s SECOND)
							=
							EXTRACT(DAY FROM FROM_UNIXTIME(" . (int)$this->currentTimestamp . ") + INTERVAL %s SECOND)
						)
						THEN 1
						ELSE 0
						END
		...
		foreach ($this->filter['MESSAGE_OF_TYPE_SENT'] as $filterItem)
		{
			...
			$minutesFilter =
				isset($filterItem['MINUTES'])
					? "
						AND CREATED_AT > NOW() - INTERVAL " . (int)$filterItem['MINUTES'] . " MINUTE
						AND CREATED_AT <= NOW()
					"
					: ""
			;
			...
			$sqlExpression = new SqlExpression("
				...
				WHERE
					...
					AND NOTIFICATION_TYPE = '" . $sqlHelper->forSql((string)$filterItem['TYPE']) . "'
					" . $minutesFilter . "
				" . $countFilter . "
			");
