...
use Bitrix\Booking\Exception\Booking\UpdateBookingException;
...
class UpdateCommandHandler
{
	...
	public function __invoke(UpdateCommand $command): Entity\Booking\Booking
	{
		...
		return $this->transactionHandler->handle(
			fn: function() use ($command, $currentBooking) {
				...
				$booking = $this->bookingRepository->getById($result->getId(), $command->updatedBy);
	...
	private function handleResources(UpdateCommand $command, Entity\Booking\Booking $booking): void
	{
	...
	private function handleClients(UpdateCommand $command, Entity\Booking\Booking $booking): void
	{
	...
	private function handleExternalData(UpdateCommand $command, Entity\Booking\Booking $booking): void
	{
	...
	//@todo we have duplicates method in Bitrix\Booking\Internals\Command\Booking\AddCommandHandler
