...
export default class LegacySharingPopup
{
			...
			BX.Disk.addToLinkParam(this.ajaxUrl, 'action', 'showSharingDetailChangeRights'),
			{
				...
				afterSuccessLoad: (response) => {
					...
					BX.Disk.modalWindow({
						...
						title: BX.message('DISK_FILE_EDITOR_ONLYOFFICE_SHARING_TITLE_MODAL_2'),
						contentClassName: '',
						contentStyle: {},
						events: {
							onAfterPopupShow: () => {
								...
								children: [
									BX.create('table', {
										...
										children: [
											BX.create('thead', {
												html: '<tr>' +
													'<td class="bx-disk-popup-shared-people-list-head-col1">' + BX.message('DISK_FILE_EDITOR_ONLYOFFICE_SHARING_LABEL_OWNER') + '</td>' +
													'</tr>'
											}),
											BX.create('tr', {
												html: '<tr>' +
									...
									BX.create('table', {
										...
										children: [
											BX.create('thead', {
												html: '<tr>' +
													'<td class="bx-disk-popup-shared-people-list-head-col1">' + BX.message('DISK_FILE_EDITOR_ONLYOFFICE_SHARING_LABEL_NAME_RIGHTS_USER') + '</td>' +
													'<td class="bx-disk-popup-shared-people-list-head-col2">' + BX.message('DISK_FILE_EDITOR_ONLYOFFICE_SHARING_LABEL_NAME_RIGHTS') + '</td>' +
									...
									BX.create('div', {
										...
										children: [
											...
											BX.create('a', {
												...
												text: BX.message('DISK_FILE_EDITOR_ONLYOFFICE_SHARING_LABEL_NAME_ADD_RIGHTS_USER'),
												events: {
													click: () => {}
												}
											})
									...
									click: () => {
										BX.Disk.ajax({
											...
											onsuccess: (response) => {
												...
												response.message = BX.message('DISK_FILE_EDITOR_ONLYOFFICE_OK_FILE_SHARE_MODIFIED').replace('#FILE#', params.object.name);
	...
	onSelectDestination(item, type, search)
	{
		...
		BX.Disk.appendNewShared({
			...
			item: item,
			type: type,
			right: this.entityToNewShared[item.id].right
		});

		this.entityToNewShared[item.id] = {
			item: item,
			type: type,
			right: this.entityToNewShared[item.id].right || 'disk_access_read'
		};
	};
