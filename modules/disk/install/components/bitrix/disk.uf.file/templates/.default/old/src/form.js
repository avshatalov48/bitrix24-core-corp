...
export default class Form extends DefaultController {
	...
	init()
	{
		...
		let itemsInsertedIntoText = [];
		this.getFileController().subscribe('onItemIsInsertedInText', ({data: item}) => {
			itemsInsertedIntoText.push(item.getId());
			this.settingsController.doNotAdviseCollageTemplate();
		});
		this.getFileController().subscribe('onItemDelete', ({data: item}) => {
			itemsInsertedIntoText = itemsInsertedIntoText.filter((value) => {
				return value != item.getId();
			});

			if (itemsInsertedIntoText.length <= 0)
			{
				this.settingsController.adviseCollageTemplate();
			}
		});
	}
	...
	initDocumentController()
	{
		...
		if (this.eventObject.dataset.bxDiskDocumentButton !== 'added')
		{
			...
			node.innerHTML = '<i></i>' + Loc.getMessage('WDUF_CREATE_DOCUMENT_MSGVER_1');
