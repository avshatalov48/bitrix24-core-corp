...
?>
		...
		$templateView = empty($arParams['arUserField']['ENTITY_VALUE_ID']) ? null : $arParams['TEMPLATE_VIEW'];
		?>
		<input name="<?= $arResult['INPUT_NAME_TEMPLATE_VIEW'] ?>" value="gallery" type="hidden" />
		<input name="<?= $arResult['INPUT_NAME_TEMPLATE_VIEW'] ?>" style="display: none;" <?php
			?>data-bx-role="settings-allow-grid" value="grid" type="checkbox" <?php
			?><?=$templateView !== 'gallery' ? 'checked ' : ''
			?><?=$templateView === null ? ' data-bx-auto="Y"' : ''?>/>
	</div>
<?
