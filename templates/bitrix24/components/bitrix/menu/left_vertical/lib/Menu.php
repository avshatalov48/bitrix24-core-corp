...
use Bitrix\Intranet\LeftMenu\MenuItem;
	...
	protected function getSavedUserMenuItems(): array
	{
		return [
			MenuItem\ItemAdminShared::class      =>
				unserialize(
					\COption::GetOptionString('intranet', 'left_menu_items_to_all_'.SITE_ID)
					, ['allowed_classes' => false]
				),
			MenuItem\ItemRestApplication::class      =>
				unserialize(
					\COption::GetOptionString('intranet', 'left_menu_items_marketplace_'.SITE_ID)
					, ['allowed_classes' => false]
				),
			MenuItem\ItemUserSelf::class          =>
				\CUserOptions::GetOption('intranet', 'left_menu_self_items_'.SITE_ID),
			MenuItem\ItemUserFavorites::class =>
				\CUserOptions::GetOption('intranet', 'left_menu_standard_items_'.SITE_ID),
		];
	}
