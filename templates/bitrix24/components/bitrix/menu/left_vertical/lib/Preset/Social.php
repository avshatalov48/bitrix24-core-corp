...
class Social extends PresetAbstract
{
	...
	public function getStructure(): array
	{
		...
		$filler = function(&$item, $key) use (&$filler, &$found) {
			...
			{
				$found = false;
				foreach (self::getMarketPlaceStructure() as $application)
				{
					$item[] = $application;
				}
			}
	...
	private static function getMarketPlaceStructure(): array
	{
		$adminOption = unserialize(\COption::GetOptionString('intranet', 'left_menu_items_marketplace_'.SITE_ID)
